...
static bool8 ShouldSwitchIfWonderGuard(void)
{
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        if (GetMonData(&party[i], 65) == 0)
            continue;
        if (GetMonData(&party[i], 65) == 898 + 314 + 1)
            continue;
...
static u8 GetAI_ItemType(u16 itemId, const u8 *itemEffect)
{
    ...
    else if ((itemEffect[0] & 0x30) || itemEffect[1])
        ...
        case AI_ITEM_X_STAT:
            *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) = 0;
            if (gDisableStructs[gActiveBattler].isFirstTurn == 0)
                break;
# 954 "src/battle_ai_switch_items.c"
            if (itemEffects[1] & 0x1)
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x1;
            if (itemEffects[1] & 0x2)
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x2;
            if (itemEffects[1] & 0x4)
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x4;
            if (itemEffects[1] & 0x8)
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x8;
            if (itemEffects[1] & 0x10)
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x10;
            if (itemEffects[1] & 0x20)
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x20;
