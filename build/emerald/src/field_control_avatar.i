...
void FieldGetPlayerInput(struct FieldInput *input, u16 newKeys, u16 heldKeys)
{
    ...
    if (heldKeys & 0x0200)
    {
        if(input->pressedSelectButton)
        {
            input->input_field_1_1 = 1;
            input->pressedSelectButton = 0;
        }else if(input->pressedStartButton)
        {
            input->input_field_1_3 = 1;
            input->pressedStartButton = 0;
        }
    }
...
static bool32 TrySetupRewindScript(void)
{
    if (FlagGet((((0x500 + 864 - 1) + 1) + 0x7)) && TrySetRewindWarp() == 2)
    {
...
static bool32 TrySetupFastForwardScript(void)
{
    if (FlagGet((((0x500 + 864 - 1) + 1) + 0x7)) && gMapHeader.mapType == 5 && TrySetRewindWarp() == 1)
    {
...
{
    if (gMapHeader.mapType == 5 && MetatileBehavior_IsFastforwardable(metatileBehavior))
    {
    ...
    else if (MetatileBehavior_IsRewindable(metatileBehavior) == 1)
    {
...
u8 TrySetRewindWarp(void)
{
    ...
    if (gMapHeader.mapType == 5 && MetatileBehavior_IsFastforwardable(metatileBehavior))
    {
        if (SetRewindWarpEmerge(x - 7, y - 7) == 1)
            return 1;
    }
    else if (MetatileBehavior_IsRewindable(metatileBehavior) == 1)
    {
        if (SetRewindWarpDive(x - 7, y - 7) == 1)
            return 2;
    }
    return 0;
}
