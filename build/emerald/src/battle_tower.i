...
static void FillPartnerParty(u16 trainerId)
{
    ...
    {
        const struct TrainerMon *partyData = gTrainers[trainerId - 3076].party.TrainerMon;
        otID = (Random() | (Random() << 16));
        ...
        for (i = 0; i < 3 && i < gTrainers[trainerId - 3076].partySize; i++)
        {
            ...
            for (j = 0; gTrainers[trainerId - 3076].trainerName[j] != 0xFF; j++)
                nameHash += gTrainers[trainerId - 3076].trainerName[j];



            if ((partyData[i].nature > 0) || (partyData[i].gender > 0))
                CreateMonWithGenderNatureLetter(&gPlayerParty[i + 3], partyData[i].species, partyData[i].lvl, fixedIV, partyData[i].gender, partyData[i].nature, 0, partyData[i].shiny ? 3 : 2);
            else
            {
                if (gTrainers[trainerId - 3076].doubleBattle == 1)
                    personalityValue = 0x80;
                else if (gTrainers[trainerId - 3076].encounterMusic_gender & 0x80)
                    personalityValue = 0x78;
                else
                    personalityValue = 0x88;

                CreateMon(&gPlayerParty[i + 3], partyData[i].species, partyData[i].lvl, fixedIV, 1, personalityValue, partyData[i].shiny ? 3 : 2, 0);
            }

            if (partyData[i].friendship == 1)
            {
                friendship = 0;
                SetMonData(&gPlayerParty[i + 3], 32, &friendship);
            }
            else if (partyData[i].friendship > 0)
                SetMonData(&gPlayerParty[i + 3], 32, &partyData[i].friendship);

            if (partyData[i].nickname[0] != '\0')
                SetMonData(&gPlayerParty[i + 3], 2, &partyData[i].nickname);
            ...
            if (partyData[i].ability > 0)
            {
                ...
                if (partyData[i].ability == 255)
                    ability = 0;

                SetMonData(&gPlayerParty[i + 3], 46, &ability);
            }


            if (partyData[i].ball > 0)
                SetMonData(&gPlayerParty[i + 3], 38, &partyData[i].ball);
            ...
            if (partyData[i].iv == 0)
            {
                for (j = 0; j < 6; j++)
                {
                    SetMonData(&gPlayerParty[i + 3], 39 + j, &partyData[i].ivs[j]);
                }
            }
            else if (partyData[i].iv == 1)
            {
                for (j = 0; j < 6; j++)
                {
                    SetMonData(&gPlayerParty[i + 3], 39 + j, 0);
                }
            }
            ...
            for (j = 0; j < 6; j++)
            {
                SetMonData(&gPlayerParty[i + 3], 26 + j, &partyData[i].evs[j]);
            }
...
s32 GetHighestLevelInPlayerParty(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (GetMonData(&gPlayerParty[i], 11, ((void *)0))
            && GetMonData(&gPlayerParty[i], 65, ((void *)0)) != 898 + 314 + 1)
        {
